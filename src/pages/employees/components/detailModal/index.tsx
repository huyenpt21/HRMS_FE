...
export default function EmployeeDetailModal({
  ...
  const { data: detailEmployee, isLoading: loadingGetDetail } =
    useEmployeeDetail(employeeRollNumber);
  const { mutate: updateEmployee, isLoading: loadingUpdate } =
    useUpdateEmployee({
  ...
  const { mutate: createEmployee, isLoading: loadingCreate } =
    useAddEmployeeModal({
      onSuccess: (response: ResEmployeeModify) => {
        const {
          metadata: { message },
  ...
  return (
    <>
      ...
      >
        <>
          {(loadingGetDetail || loadingUpdate || loadingCreate) && (
      ...
      {isShowRegisterModal && (
        <SignatureRegisterModal
          ...
          employee={personInforRef.current}
        />
      )}
    </>
  );
